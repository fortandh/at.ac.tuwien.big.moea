...
import at.ac.tuwien.big.momot.search.algorithm.MyMOPSO;
...
public class EvolutionaryAlgorithmFactory<S extends Solution> extends AbstractAlgorithmFactory<S> {
   ...
   // just for the experiment
   public IRegisteredAlgorithm<MyEMOEA> createMyEMOEA() {
      return createMyEMOEA(DEFAULT_EPSILON, new TournamentSelection(2), new OnePointCrossover(1.0),
            new PlaceholderMutation(DEFAULT_MUTATION_PROBABILITY));
   }

   // just for the experiment
   public IRegisteredAlgorithm<MyEMOEA> createMyEMOEA(final double epsilon, final Selection selection,
         final Variation... variation) {
      return new AbstractRegisteredAlgorithm<MyEMOEA>() {
         @Override
         public MyEMOEA createAlgorithm() {
            return new MyEMOEA(createProblem(), createPopulation(), createEpsilonBoxArchive(epsilon), selection,
                  createVariation(variation), createInitialization());
         }
      };
   }
   ...
   // just for the experiment
   public IRegisteredAlgorithm<MyMOPSO> createMyMOPSO() {
      return createMyMOPSO(new CrowdingComparator(), new ParetoDominanceComparator(),
            new PlaceholderMutation(DEFAULT_MUTATION_PROBABILITY));
   }

   // just for the experiment
   public IRegisteredAlgorithm<MyMOPSO> createMyMOPSO(final DominanceComparator leaderComparator,
         final DominanceComparator dominanceComparator, final Variation mutation) {
      return new AbstractRegisteredAlgorithm<MyMOPSO>() {
         @Override
         public MyMOPSO createAlgorithm() {
            return new MyMOPSO(createProblem(), getPopulationSize(), getPopulationSize() / 10, leaderComparator,
                  dominanceComparator, new NondominatedPopulation(), mutation, createInitialization());
         }
      };
   }

   // just for the experiment
   public IRegisteredAlgorithm<MyMOPSO> createMyMOPSO(final Variation mutation) {
      return createMyMOPSO(new CrowdingComparator(), new ParetoDominanceComparator(), mutation);
   }

   public IRegisteredAlgorithm<MyNSGAII> createMyNSGAII() {
      return createMyNSGAII(new TournamentSelection(2), new OnePointCrossover(1.0),
            new PlaceholderMutation(DEFAULT_MUTATION_PROBABILITY));
   }

   public IRegisteredAlgorithm<MyNSGAII> createMyNSGAII(final Selection selection, final Variation... variation) {
      return new AbstractRegisteredAlgorithm<MyNSGAII>() {
         @Override
         public MyNSGAII createAlgorithm() {
            return new MyNSGAII(createProblem(), createSortingPopulation(), createEpsilonBoxArchive(), selection,
                  createVariation(variation), createInitialization());
         }
      };
   }
